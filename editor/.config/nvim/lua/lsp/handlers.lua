...
function M.setup()
  local config = { -- your config
    virtual_text = olsp.diagnostics.virtual_text,
    signs = olsp.diagnostics.signs,
    underline = olsp.diagnostics.underline,
    update_in_insert = olsp.diagnostics.update_in_insert,
    severity_sort = olsp.diagnostics.severity_sort,
  }
  if vim.fn.has("nvim-0.5.1") > 0 then
    vim.lsp.handlers["textDocument/publishDiagnostics"] = function(_, result, ctx, _)
      ...
      if ok then
        ...
        for i, sign in ipairs(olsp.diagnostics.signs.values) do
